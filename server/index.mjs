...
import { LAB3_CONTEXT_ENGINE_VERSION, decorateEdhrecThemesForLab3 } from "./experimental/lab3/deck-context.mjs";
...
// Serves the built React client (npm run build inside client/). In development the client
// runs its own Vite dev server and proxies /api/* to this server instead (see client/vite.config.js).
...
const COMMANDER_SEARCH_PROFILE_TTL_MS=15*60*1000;
const commanderSearchProfileCache=new Map();
function rememberCommanderSearchProfile(row){
  const key=cardIdentityKey(row?.name);if(!key)return;
  commanderSearchProfileCache.set(key,{row,updatedAt:Date.now()});
  if(commanderSearchProfileCache.size>160){
    const oldest=[...commanderSearchProfileCache.entries()].sort((a,b)=>Number(a[1]?.updatedAt||0)-Number(b[1]?.updatedAt||0)).slice(0,commanderSearchProfileCache.size-120);
    for(const [k] of oldest)commanderSearchProfileCache.delete(k);
  }
}
function recentCommanderSearchProfile(name){
  const key=cardIdentityKey(name),hit=commanderSearchProfileCache.get(key);if(!hit)return null;
  if(Date.now()-Number(hit.updatedAt||0)>COMMANDER_SEARCH_PROFILE_TTL_MS){commanderSearchProfileCache.delete(key);return null}
  return hit.row||null;
}
function commanderSearchRow(c){
  return {
    ...
    colorIdentity:c.color_identity||[],cmc:Number(c.cmc||0),keywords:c.keywords||[],edhrecRank:c.edhrec_rank??null,legalities:c.legalities||{}
  };
}
async function commanderSearch(q){
  ...
  const d=await r.json(),rows=(d.data||[]).slice(0,12).map(commanderSearchRow);
  for(const row of rows)rememberCommanderSearchProfile(row);
  return rows;
}
...
async function externalFieldQuality({engine,deckRows,commanderSemantic,themeName,themeModel,themeTarget=null}){
  const rows=Array.isArray(deckRows)?deckRows:[],names=[...new Set(rows.map(x=>x?.name).filter(Boolean))],resolved=await lab3Catalog.getMany(names),cards=[],externalThemeEvidenceByName=new Map();let resolvedCount=0,total=0;
  for(const row of rows){
    const qty=Math.max(1,Number(row?.quantity||1)),card=resolved.get(row.name)||await lab3Catalog.get(row.name);total+=qty;
    // LAB3 composite membership can legitimately require independent EDHREC corroboration.
    // Deck Quality re-profiles Runtime cards, so carry forward only the structured external
    // theme score emitted by the Builder. External evidence alone still cannot create
    // membership because themeMembershipForProfile keeps the semantic-family requirement.
    const externalThemeScore=Number(row?.externalThemeScore);if(Number.isFinite(externalThemeScore)&&externalThemeScore>0)externalThemeEvidenceByName.set(String(row.name||"").toLocaleLowerCase("en-US"),Math.max(Number(externalThemeEvidenceByName.get(String(row.name||"").toLocaleLowerCase("en-US"))||0),externalThemeScore));
    if(card){resolvedCount+=qty;for(let i=0;i<qty;i++)cards.push(card);}
  }
  return {quality:evaluateDeckQuality({engine,cards,commander:commanderSemantic,theme:themeName,themeModel,themeTarget,externalThemeEvidenceByName}),resolvable:resolvedCount===total&&total===100,cardCount:total,resolvedCount};
}
async function buildLab3(session,{commander,theme,settings}={}){
  ...
  const runtimeStatus=await lab3Catalog.status(),legacyInteraction=(health?.health||[]).find(x=>x?.key==="interaction")||null;
  const diagnostic={schema:`manashelf-lab3-build-log-v${LAB3_BUILDER_VERSION}`,generatedAt:new Date().toISOString(),appVersion:APP_VERSION,engines:{builderVersion:LAB3_BUILDER_VERSION,manaModelVersion:LAB3_MANA_MODEL_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,comboEngineVersion:COMBO_ENGINE_VERSION,runtime:runtimeStatus.header?.engineVersions||null,comparisonHealthSemanticSource:`classification-v${CLASSIFICATION_VERSION}`},input:{commander:String(commander),theme:{...chosen,themeContractMode:result.themeMode||themeMode,themeUnderstanding:result.themeUnderstanding||null},settings:result.settings},pool:{collectionCards:collection.length,semanticMatched:runtime.size,candidates:candidates.length,excluded:exclusions.length,excludedPrintings:printingExclusions.length,spellbook:{source:spellbook.source,query:spellbook.query,policy:comboPolicy,...spellbookSummary},exclusions,printingExclusions},result,comparison:{deckHealth:health,metricBridge:{interaction:{builderSemantic:Number(result.summary?.roleCounts?.interaction||0),deckHealthLegacyWeighted:legacyInteraction?Number(legacyInteraction.value||0):null,deckHealthDetected:health?.context?Number(health.context.interaction||0):null,directlyComparable:false,reason:"LAB3 usa roles Semantic v2 ponderados; Deck Health conserva Classification legacy para comparación con LAB2."},dependency:{allCardsAverage:Number(result.context?.summary?.avgDependencyReliability||0),dependentCardsAverage:Number(result.context?.summary?.avgDependencySatisfaction||0),dependentCards:Number(result.context?.summary?.dependentCards||0),bottlenecks:Number(result.context?.bottlenecks?.length||0)}}}};
  ...
  try{const fq=await externalFieldQuality({engine:"vnext",deckRows:result.deck,commanderSemantic,themeName:chosen.name,themeModel:result.themeModel||trustedThemeModel,themeTarget:result.targets?.theme}),cf=fieldCollectionFingerprint(collection);await fieldQualityRegistry.record({engine:"vnext",commander:String(commander),theme:chosen.name,settings:settings||{},collectionFingerprint:cf,quality:fq.quality,resolvable:fq.resolvable,cardCount:fq.cardCount,metadata:{builderVersion:LAB3_BUILDER_VERSION,themeMode:result.themeMode||themeMode}});diagnostic.fieldQuality=fq.quality;}catch(e){diagnostic.fieldQualityError=String(e.message||e);}
  ...
  return {builderVersion:LAB3_BUILDER_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,themeSource:themeFetch.source,themeContractMode:result.themeMode||themeMode,candidateCount:candidates.length,exclusionCount:exclusions.length,diagnosticLogId:id,build:result,deckDetail:detail,health,runtime:runtimeStatus,warnings:[themeFetch.warning,spellbook.warning].filter(Boolean)};
}
...
// ---- LAB 3 · Fixed unseen holdout + current-engine recertification ---------------
// Validation-only. This fixed plan is historical holdout evidence and must never be
// consumed by Theme Corpus training. The runtime verifies zero Commander overlap
// against the current Stress v4 commander selection before executing it.
const LAB3_UNSEEN_SCHEMA="manashelf-lab3-unseen-holdout-v1";
const LAB3_UNSEEN_VERSION=1;
const LAB3_UNSEEN_PLAN_FILE=path.join(__dirname,"experimental/lab3/validation/unseen-holdout-v1.json");
let lab3UnseenPlanCache=null;
async function loadLab3UnseenPlan(){
  if(lab3UnseenPlanCache)return lab3UnseenPlanCache;
  const parsed=JSON.parse(await fs.readFile(LAB3_UNSEEN_PLAN_FILE,"utf8")),cases=Array.isArray(parsed?.cases)?parsed.cases:[];
  if(cases.length!==60)throw new Error(`Unseen holdout inválido: se esperaban 60 casos y hay ${cases.length}.`);
  const ids=new Set(),commanders=new Set();
  for(const row of cases){if(!row?.id||!row?.commander||!row?.commanderOracleId||!row?.theme)throw new Error("Unseen holdout incompleto.");if(ids.has(row.id))throw new Error(`Unseen holdout con id duplicado: ${row.id}`);if(commanders.has(row.commanderOracleId))throw new Error(`Unseen holdout repite Commander: ${row.commander}`);ids.add(row.id);commanders.add(row.commanderOracleId);}
  lab3UnseenPlanCache={schema:LAB3_UNSEEN_SCHEMA,version:LAB3_UNSEEN_VERSION,seed:String(parsed.seed||"manashelf-lab3-stress-holdout-1"),seedNumber:Number(parsed.seedNum||hashSeed(parsed.seed||"manashelf-lab3-stress-holdout-1")),cases};return lab3UnseenPlanCache;
}
function publicUnseenStatus(job){if(!job)return {active:false,status:"idle",message:"Unseen holdout no iniciado."};return {active:job.status==="planning"||job.status==="running",status:job.status,message:job.message,requested:job.requested,planned:job.plan?.length||0,completed:job.completed||0,failed:job.failed||0,current:job.current||null,startedAt:job.startedAt,finishedAt:job.finishedAt||null,seed:job.seed,summary:job.summary||null,clusters:(job.clusters||[]).slice(0,12),overlapWithStress:Number(job.overlapWithStress||0),exportReady:job.status==="done"||job.status==="done_with_errors"};}
async function runLab3Unseen(session,{runs=60,stressSeed="manashelf-lab3-stress-4"}={}){
  runs=Math.max(1,Math.min(60,Math.round(Number(runs)||60)));const fixed=await loadLab3UnseenPlan(),job={schema:LAB3_UNSEEN_SCHEMA,version:LAB3_UNSEEN_VERSION,corpusMode:"fixed_unseen_holdout",status:"planning",message:"Validando holdout fijo y separación respecto de Stress v4…",requested:runs,seed:fixed.seed,seedNumber:fixed.seedNumber,startedAt:new Date().toISOString(),completed:0,failed:0,cases:[],plan:fixed.cases.slice(0,runs),snapshots:{baseByCommander:{},themeByCase:{}},cancelRequested:false};session.lab3UnseenJob=job;
  try{
    const idx=await lab3Catalog.ensureLoaded(),runtimeCards=[...idx.byOracleId.values()],globalPool=stressGlobalPool(runtimeCards),stressCommanderCount=Math.min(150,Math.ceil(200/3)+14),stressCommanderSeed=hashSeed(stressSeed),stressCommanders=selectStressCommanders({runtimeCards,poolCards:globalPool,commanderCount:stressCommanderCount,seed:stressCommanderSeed,minPool:140}),stressIds=new Set(stressCommanders.map(c=>String(c.oracleId||""))),stressNames=new Set(stressCommanders.map(c=>cardIdentityKey(c.name))),overlap=job.plan.filter(x=>stressIds.has(String(x.commanderOracleId||""))||stressNames.has(cardIdentityKey(x.commander)));job.overlapWithStress=overlap.length;job.globalPoolSnapshot=stressPoolSnapshot(globalPool);job.runtimeStatus=await lab3Catalog.status();job.poolStats={runtimeCards:runtimeCards.length,globalPlayableNonbasic:globalPool.length,holdoutCases:job.plan.length,stressCommanderReference:stressCommanders.length,overlapWithStress:overlap.length};
    if(overlap.length)throw new Error(`Unseen holdout dejó de ser independiente: ${overlap.length} Commanders solapan con Stress v4.`);
    job.status="running";job.message=`Ejecutando ${job.plan.length} builds unseen contra el pool global…`;const commanderCache=new Map();
    for(let i=0;i<job.plan.length;i++){
      if(job.cancelRequested){job.status="cancelled";job.message="Unseen holdout cancelado por el usuario.";break}
      const planned=job.plan[i],profile=stressSettingsFor(i),theme={name:planned.theme,slug:slug(planned.theme),themeContractMode:"semantic",semanticSupported:true,semanticContractKey:planned.facet||null};job.current={index:i+1,total:job.plan.length,commander:planned.commander,theme:planned.theme};job.message=`LAB3 unseen ${i+1}/${job.plan.length} · ${planned.commander} · ${planned.theme}`;
      try{
        let common=commanderCache.get(planned.commanderOracleId);if(!common){const commander=idx.byOracleId.get(planned.commanderOracleId)||await lab3Catalog.get(planned.commander);if(!commander)throw new Error("Commander del holdout ausente del Runtime certificado");const baseLists=await edhrec(commander.name).catch(()=>[]);common={commander,baseLists};commanderCache.set(planned.commanderOracleId,common);job.snapshots.baseByCommander[commander.name]=baseLists;}
        const themeFetch=await edhrecThemeLists(common.commander.name,theme).catch(e=>({lists:[],source:"unavailable",warning:String(e.message||e)})),themeLists=themeFetch.lists||[];job.snapshots.themeByCase[planned.id]={source:themeFetch.source,warning:themeFetch.warning||null,lists:themeLists};const candidates=stressCandidatesFor({poolCards:globalPool,commander:common.commander,baseLists:common.baseLists,themeLists}),trustedThemeModel=await themeCorpusCatalog.getTrustedModel(planned.theme).catch(()=>null),build=buildVNextDeck({commander:common.commander,theme:planned.theme,themeMode:"semantic",themeModel:trustedThemeModel,candidates,settings:profile.settings}),actualThemeMode=build.themeMode||"semantic",semanticByName=stressSemanticMap(common.commander,candidates),anomalies=analyzeStressBuild({build,semanticByName,settings:profile.settings,themeMode:actualThemeMode}),modal=stressModalStats(build,semanticByName),row=compactStressCase({id:planned.id,commander:common.commander,theme,themeMode:actualThemeMode,settingsProfile:profile.profile,settings:profile.settings,build,anomalies,candidateCount:candidates.length,themeSource:themeFetch.source,modal});job.cases.push({...row,corpusMode:"fixed_unseen_holdout",commanderOracleId:planned.commanderOracleId,holdoutFacet:planned.facet||null});job.completed++;
      }catch(e){job.failed++;const commander=idx.byOracleId.get(planned.commanderOracleId)||{name:planned.commander,colorIdentity:[]};const row=compactStressCase({id:planned.id,commander,theme,themeMode:"semantic",settingsProfile:profile.profile,settings:profile.settings,status:"error",error:String(e.message||e),candidateCount:0});job.cases.push({...row,corpusMode:"fixed_unseen_holdout",commanderOracleId:planned.commanderOracleId,holdoutFacet:planned.facet||null});}
      if((i+1)%5===0||i===job.plan.length-1){job.clusters=clusterStressIssues(job.cases);job.summary=summarizeStressCases(job.cases);}
    }
    if(job.status!=="cancelled"){job.status=job.failed?"done_with_errors":"done";job.message=job.failed?`Unseen terminado con ${job.failed} builds fallidos.`:"Unseen holdout 60 terminado.";}job.finishedAt=new Date().toISOString();job.clusters=clusterStressIssues(job.cases);job.summary=summarizeStressCases(job.cases);
  }catch(e){job.status="error";job.message=String(e.message||e);job.error=String(e.stack||e);job.finishedAt=new Date().toISOString();}
  return job;
}
async function lab3UnseenZip(job){
  if(!job||!["done","done_with_errors","cancelled"].includes(job.status))throw new Error("El unseen holdout todavía no está listo para exportar.");const generatedAt=new Date().toISOString(),themeCorpus=await themeCorpusCatalog.status(),knowledge=await knowledgeCatalog.status(),vnext=builderVNextStatus(),manifest={schema:LAB3_UNSEEN_SCHEMA,unseenVersion:LAB3_UNSEEN_VERSION,corpusMode:"fixed_unseen_holdout",generatedAt,appVersion:APP_VERSION,builderVersion:LAB3_BUILDER_VERSION,manaModelVersion:LAB3_MANA_MODEL_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,requested:job.requested,seed:job.seed,seedNumber:job.seedNumber,status:job.status,runtime:job.runtimeStatus,themeCorpus,knowledge,vnext,poolStats:job.poolStats||null,validationOnly:true,trainingAllowed:false,independentFromStress:Number(job.overlapWithStress||0)===0,replayInstructions:"Use plan.json + edhrec-snapshots.json + global-pool.json with the same Runtime + Theme Corpus + Knowledge hashes. This is validation-only evidence and must never train Theme Corpus."},casesJsonl=(job.cases||[]).map(x=>JSON.stringify(x)).join("\n")+"\n",entries=[{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"summary.json",data:JSON.stringify(job.summary||{},null,2)},{name:"clusters.json",data:JSON.stringify(job.clusters||[],null,2)},{name:"plan.json",data:JSON.stringify(job.plan||[],null,2)},{name:"cases.jsonl",data:casesJsonl},{name:"global-pool.json",data:JSON.stringify(job.globalPoolSnapshot||[],null,2)},{name:"edhrec-snapshots.json",data:JSON.stringify(job.snapshots||{},null,2)}];return createZip(entries);
}
const LAB3_FIELD_PRECHECK_SCHEMA="manashelf-lab3-field-precheck-v1";
const LAB3_FIELD_PRECHECK_VERSION=1;
const LAB3_FIELD_PRECHECK_FILE=path.join(__dirname,"experimental/lab3/validation/field-precheck-v1.json");
let lab3FieldPrecheckPlanCache=null;
async function loadLab3FieldPrecheckPlan(){
  if(lab3FieldPrecheckPlanCache)return lab3FieldPrecheckPlanCache;
  const parsed=JSON.parse(await fs.readFile(LAB3_FIELD_PRECHECK_FILE,"utf8")),cases=Array.isArray(parsed?.cases)?parsed.cases:[],gates=parsed?.gates||{};
  if(parsed?.schema!==LAB3_FIELD_PRECHECK_SCHEMA||Number(parsed?.version)!==LAB3_FIELD_PRECHECK_VERSION)throw new Error("Plan field pre-check LAB3 incompatible.");
  if(parsed?.validationOnly!==true||parsed?.trainingAllowed!==false)throw new Error("El field pre-check debe permanecer validation-only y fuera de training.");
  if(cases.length!==3)throw new Error(`Field pre-check inválido: se esperaban 3 casos y hay ${cases.length}.`);
  for(const row of cases){if(!row?.id||!row?.commander||!row?.theme?.name||!row?.settings)throw new Error("Field pre-check incompleto.");}
  lab3FieldPrecheckPlanCache={schema:parsed.schema,version:Number(parsed.version),validationOnly:true,trainingAllowed:false,gates,cases};return lab3FieldPrecheckPlanCache;
}
function evaluateFieldPrecheckDiagnostic(diagnostic,gates={}){
  const result=diagnostic?.result||{},quality=diagnostic?.fieldQuality||{},metrics=quality?.metrics||{},checks={
    deckSize:Number(result?.size||0)===Number(gates.deckSize||100),
    themeFidelity:Number(metrics.themeFidelity??-1)>=Number(gates.minThemeFidelity??.8),
    structuralCoverage:Number(metrics.structuralCoverage??-1)>=Number(gates.minStructuralCoverage??.95),
    manaReliability:Number(metrics.manaReliability??-1)>=Number(gates.minManaReliability??.95),
    dependencySatisfaction:Number(metrics.dependencySatisfaction??-1)>=Number(gates.minDependencySatisfaction??.8)
  };
  return {pass:Object.values(checks).every(Boolean),checks,metrics:{themeFidelity:metrics.themeFidelity??null,structuralCoverage:metrics.structuralCoverage??null,manaReliability:metrics.manaReliability??null,dependencySatisfaction:metrics.dependencySatisfaction??null,score:quality?.score??null}};
}
function publicLab3FieldPrecheckStatus(job){if(!job)return {status:"idle",requested:3,completed:0,failed:0,cases:[]};return {status:job.status,requested:Number(job.requested||0),completed:Number(job.completed||0),failed:Number(job.failed||0),current:job.current||null,message:job.message||null,cases:(job.cases||[]).map(x=>({id:x.id,commander:x.commander,theme:x.theme,pass:x.pass,checks:x.checks,metrics:x.metrics,error:x.error||null})),summary:job.summary||null};}
async function runLab3FieldPrecheck(session){
  const plan=await loadLab3FieldPrecheckPlan(),job={schema:plan.schema,version:plan.version,status:"running",message:"Ejecutando field pre-check LAB3 · 3 builds con la colección conectada…",requested:plan.cases.length,completed:0,failed:0,cases:[],plan:plan.cases,gates:plan.gates,startedAt:new Date().toISOString(),cancelRequested:false};session.lab3FieldPrecheckJob=job;if(session.lab3RecertJob)session.lab3RecertJob.precheck=job;
  for(let i=0;i<plan.cases.length;i++){
    if(job.cancelRequested){job.status="cancelled";job.message="Field pre-check cancelado por el usuario.";break}
    const planned=plan.cases[i];job.current={index:i+1,total:plan.cases.length,commander:planned.commander,theme:planned.theme.name};job.message=`Field pre-check ${i+1}/${plan.cases.length} · ${planned.commander} · ${planned.theme.name}`;
    try{
      const built=await buildLab3(session,{commander:planned.commander,theme:planned.theme,settings:planned.settings}),diagnostic=session.lab3BuildLogs?.get?.(built.diagnosticLogId)||null,gate=evaluateFieldPrecheckDiagnostic(diagnostic,plan.gates),row={id:planned.id,commander:planned.commander,theme:planned.theme.name,settings:planned.settings,diagnosticLogId:built.diagnosticLogId,pass:gate.pass,checks:gate.checks,metrics:gate.metrics,diagnostic};job.cases.push(row);if(gate.pass)job.completed++;else job.failed++;
    }catch(e){job.failed++;job.cases.push({id:planned.id,commander:planned.commander,theme:planned.theme.name,settings:planned.settings,pass:false,checks:{build:false},metrics:{},error:String(e.message||e),diagnostic:null});}
    if(job.failed){const currentRow=job.cases[job.cases.length-1],failedChecks=Object.entries(currentRow?.checks||{}).filter(([,ok])=>!ok).map(([key])=>key);job.message=`Field pre-check ${i+1}/${plan.cases.length} · ${planned.commander} / ${planned.theme.name} no pasó ${failedChecks.join(", ")||"los gates"}; continuando con los demás casos…`;}
  }
  if(job.status==="running"){job.status=job.failed?"failed":"done";job.message=job.failed?`Field pre-check completo · ${job.completed}/${job.requested} PASS · ${job.failed} fallidos. Stress/unseen no se ejecutarán.`:"Field pre-check 3/3 PASS.";}
  job.finishedAt=new Date().toISOString();job.current=null;job.summary={decision:job.status==="done"&&job.completed===job.requested&&job.failed===0?"PASS":"FAIL",completed:job.completed,failed:job.failed,requested:job.requested,gates:job.gates,cases:job.cases.map(x=>({id:x.id,commander:x.commander,theme:x.theme,pass:x.pass,checks:x.checks,metrics:x.metrics,error:x.error||null}))};return job;
}
async function lab3FieldPrecheckZip(job){
  if(!job||!["done","failed","cancelled"].includes(job.status))throw new Error("El field pre-check todavía no está listo para exportar.");
  const manifest={schema:job.schema,version:job.version,generatedAt:new Date().toISOString(),appVersion:APP_VERSION,builderVersion:LAB3_BUILDER_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,manaModelVersion:LAB3_MANA_MODEL_VERSION,status:job.status,validationOnly:true,trainingAllowed:false,gates:job.gates,summary:job.summary},entries=[{name:"manifest.json",data:JSON.stringify(manifest,null,2)},{name:"plan.json",data:JSON.stringify(job.plan||[],null,2)},{name:"summary.json",data:JSON.stringify(job.summary||{},null,2)}];
  for(const [i,row] of (job.cases||[]).entries()){const base=`${String(i+1).padStart(2,"0")}-${slug(row.commander)}-${slug(row.theme)}`;entries.push({name:`cases/${base}.json`,data:JSON.stringify({id:row.id,commander:row.commander,theme:row.theme,settings:row.settings,pass:row.pass,checks:row.checks,metrics:row.metrics,error:row.error||null},null,2)});if(row.diagnostic)entries.push({name:`diagnostics/${base}-Build-Diagnostic.json`,data:JSON.stringify(row.diagnostic,null,2)});}
  return createZip(entries);
}
function recertChildStats(job){const summary=job?.summary||{};return {status:job?.status||"idle",completed:Number(job?.completed||0),failed:Number(job?.failed||0),anomalous:Number(summary.anomalous||0),clean:Number(summary.clean||0),requested:Number(job?.requested||0)};}
function publicLab3RecertStatus(session){
  const job=session?.lab3RecertJob;if(!job)return {active:false,status:"idle",stage:"idle",message:"Recertificación no iniciada.",processed:0,total:263,failed:0,anomalous:0,precheck:publicLab3FieldPrecheckStatus(null),exportReady:false};
  const precheck=recertChildStats(job.precheck),stress=recertChildStats(job.stress),unseen=recertChildStats(job.unseen),processed=precheck.completed+precheck.failed+stress.completed+stress.failed+unseen.completed+unseen.failed,failed=precheck.failed+stress.failed+unseen.failed,anomalous=stress.anomalous+unseen.anomalous,clusters=[...(job.stress?.clusters||[]),...(job.unseen?.clusters||[])].reduce((map,row)=>{const prev=map.get(row.family)||{...row,count:0,cases:[],examples:[]};prev.count+=Number(row.count||0);prev.cases=[...new Set([...(prev.cases||[]),...(row.cases||[])])].slice(0,30);prev.examples=[...(prev.examples||[]),...(row.examples||[])].slice(0,5);map.set(row.family,prev);return map;},new Map());
  return {active:["planning","precheck","stress","unseen"].includes(job.status),status:job.status,stage:job.stage||job.status,message:job.message,processed,total:263,failed,anomalous,precheck:publicLab3FieldPrecheckStatus(job.precheck),stress,unseen,startedAt:job.startedAt,finishedAt:job.finishedAt||null,summary:job.summary||null,clusters:[...clusters.values()],exportReady:job.status==="done"||job.status==="failed"};
}
async function runLab3Recertification(session,{stressSeed="manashelf-lab3-stress-4"}={}){
  const current=session.lab3RecertJob;if(current&&["planning","precheck","stress","unseen"].includes(current.status))throw new Error("Ya hay una recertificación LAB 3 en ejecución.");const job={schema:"manashelf-lab3-recertification-v2",version:2,status:"planning",stage:"planning",message:"Preparando recertificación LAB 3 · field pre-check + Stress + unseen…",startedAt:new Date().toISOString(),cancelRequested:false,precheck:null,stress:null,unseen:null,summary:null};session.lab3RecertJob=job;
  try{
    job.status="precheck";job.stage="field-precheck-3";job.message="Ejecutando field pre-check · Bruna / Kotis / Animar…";const precheckPromise=runLab3FieldPrecheck(session);job.precheck=session.lab3FieldPrecheckJob;await precheckPromise;job.precheck=session.lab3FieldPrecheckJob;if(job.cancelRequested||job.precheck?.status==="cancelled"){job.status="cancelled";job.stage="cancelled";job.message="Recertificación cancelada durante field pre-check.";job.finishedAt=new Date().toISOString();return job;}
    const precheckPass=job.precheck?.status==="done"&&Number(job.precheck?.completed||0)===3&&Number(job.precheck?.failed||0)===0;if(!precheckPass){job.status="failed";job.stage="precheck-failed";job.message=`Field pre-check no pasó (${Number(job.precheck?.completed||0)}/3 PASS · ${Number(job.precheck?.failed||0)} fallidos); Stress/unseen no se ejecutaron.`;job.summary={decision:"FAIL",precheck:{pass:false,...(job.precheck?.summary||{})},stress:{pass:false,status:"not_run"},unseen:{pass:false,status:"not_run"},engine:{builderVersion:LAB3_BUILDER_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,manaModelVersion:LAB3_MANA_MODEL_VERSION,appVersion:APP_VERSION}};job.finishedAt=new Date().toISOString();return job;}
    job.status="stress";job.stage="stress-200";job.message="Field pre-check 3/3 PASS; ejecutando Stress v4 · 200 builds…";const stressPromise=runLab3Stress(session,{runs:200,seed:stressSeed});job.stress=session.lab3StressJob;await stressPromise;if(job.cancelRequested||job.stress?.status==="cancelled"){job.status="cancelled";job.stage="cancelled";job.message="Recertificación cancelada durante Stress.";job.finishedAt=new Date().toISOString();return job;}
    if(!["done","done_with_errors"].includes(job.stress?.status)){job.status="failed";job.stage="stress-error";job.message=`Stress no pudo completarse · ${job.stress?.message||"error desconocido"}`;job.summary={decision:"FAIL",precheck:{pass:true,...(job.precheck?.summary||{})},stress:{pass:false,status:job.stress?.status||"error",error:job.stress?.error||job.stress?.message||null},unseen:{pass:false,status:"not_run"},engine:{builderVersion:LAB3_BUILDER_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,manaModelVersion:LAB3_MANA_MODEL_VERSION,appVersion:APP_VERSION}};job.finishedAt=new Date().toISOString();return job;}
    job.status="unseen";job.stage="unseen-60";job.message="Stress finalizado; ejecutando unseen holdout · 60 builds…";const unseenPromise=runLab3Unseen(session,{runs:60,stressSeed});job.unseen=session.lab3UnseenJob;await unseenPromise;job.unseen=session.lab3UnseenJob;if(job.cancelRequested||job.unseen?.status==="cancelled"){job.status="cancelled";job.stage="cancelled";job.message="Recertificación cancelada durante unseen.";job.finishedAt=new Date().toISOString();return job;}
    if(!["done","done_with_errors"].includes(job.unseen?.status)){job.status="failed";job.stage="unseen-error";job.message=`Unseen no pudo completarse · ${job.unseen?.message||"error desconocido"}`;job.summary={decision:"FAIL",precheck:{pass:true,...(job.precheck?.summary||{})},stress:{pass:false,...(job.stress?.summary||{})},unseen:{pass:false,status:job.unseen?.status||"error",error:job.unseen?.error||job.unseen?.message||null,overlapWithStress:Number(job.unseen?.overlapWithStress||0)},engine:{builderVersion:LAB3_BUILDER_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,manaModelVersion:LAB3_MANA_MODEL_VERSION,appVersion:APP_VERSION}};job.finishedAt=new Date().toISOString();return job;}
    const stressSummary=job.stress?.summary||{},unseenSummary=job.unseen?.summary||{},stressPass=Number(stressSummary.completed||0)===200&&Number(stressSummary.failed||0)===0&&Number(stressSummary.anomalous||0)===0,unseenPass=Number(unseenSummary.completed||0)===60&&Number(unseenSummary.failed||0)===0&&Number(unseenSummary.anomalous||0)===0&&Number(job.unseen?.overlapWithStress||0)===0,decision=precheckPass&&stressPass&&unseenPass?"PASS":"FAIL";job.summary={decision,precheck:{pass:precheckPass,...(job.precheck?.summary||{})},stress:{pass:stressPass,...stressSummary},unseen:{pass:unseenPass,overlapWithStress:Number(job.unseen?.overlapWithStress||0),...unseenSummary},engine:{builderVersion:LAB3_BUILDER_VERSION,contextEngineVersion:LAB3_CONTEXT_ENGINE_VERSION,manaModelVersion:LAB3_MANA_MODEL_VERSION,appVersion:APP_VERSION}};job.status=decision==="PASS"?"done":"failed";job.stage="complete";job.message=decision==="PASS"?"Recertificación 3/3 + 200/200 + 60/60 PASS.":"Recertificación terminada con fallas o anomalías; revisar evidencia.";job.finishedAt=new Date().toISOString();
  }catch(e){job.status="failed";job.stage="error";job.message=String(e.message||e);job.error=String(e.stack||e);job.finishedAt=new Date().toISOString();}
  return job;
}
function recertFailureArtifact(label,child){return {name:`${label}-error.json`,data:JSON.stringify({status:child?.status||"not_run",message:child?.message||null,error:child?.error||null,summary:child?.summary||null,completed:Number(child?.completed||0),failed:Number(child?.failed||0)},null,2)};}
async function lab3RecertificationZip(job){
  if(!job||!["done","failed"].includes(job.status))throw new Error("La recertificación todavía no está lista para exportar.");
  const manifest={schema:job.schema,version:job.version,generatedAt:new Date().toISOString(),appVersion:APP_VERSION,status:job.status,summary:job.summary,validationOnly:true,trainingAllowed:false,evidenceSeparation:{fieldPrecheck:"connected_collection_validation",stress:"global_runtime_stress_v4",unseen:"fixed_holdout_v1"}},entries=[{name:"recertification-manifest.json",data:JSON.stringify(manifest,null,2)}];
  if(["done","failed","cancelled"].includes(job.precheck?.status))entries.push({name:"field-precheck-3.zip",data:await lab3FieldPrecheckZip(job.precheck)});else entries.push(recertFailureArtifact("field-precheck-3",job.precheck));
  if(["done","done_with_errors","cancelled"].includes(job.stress?.status))entries.push({name:"stress-200.zip",data:await lab3StressZip(job.stress)});else entries.push(recertFailureArtifact("stress-200",job.stress));
  if(["done","done_with_errors","cancelled"].includes(job.unseen?.status))entries.push({name:"unseen-60.zip",data:await lab3UnseenZip(job.unseen)});else entries.push(recertFailureArtifact("unseen-60",job.unseen));
  return createZip(entries);
}

const LAB2_PROFILE_SCRYFALL_TIMEOUT_MS=3500;
const LAB2_PROFILE_EDHREC_TIMEOUT_MS=4500;
function searchProfileToMeta(row){
  if(!row)return null;
  return {canonicalName:row.name,small:row.image||null,normal:row.image||null,large:row.largeImage||row.image||null,typeLine:row.typeLine||"",oracleText:row.oracleText||"",cmc:Number(row.cmc||0),manaCost:row.manaCost||"",producedMana:[],colorIdentity:row.colorIdentity||[],keywords:row.keywords||[],edhrecRank:row.edhrecRank??null,legalities:row.legalities||{}};
}
async function fetchLab2CommanderProfileMeta(name,{timeoutMs=LAB2_PROFILE_SCRYFALL_TIMEOUT_MS}={}){
  const recent=recentCommanderSearchProfile(name);if(recent)return {meta:searchProfileToMeta(recent),source:"commander-search-cache"};
  const disk=await loadScryfallDiskCache(),cached=objectCardAliasGet(disk.cards,name);
...
async function lab2CommanderProfile(session,commander,{reuseCached=false}={}){
  ...
  setLab2Progress(session,"profile","Validando Commander y cargando themes en paralelo…",{commander:name});
  const [metaResult,tagsResult]=await Promise.allSettled([
    fetchLab2CommanderProfileMeta(name,{timeoutMs:LAB2_PROFILE_SCRYFALL_TIMEOUT_MS}),
    edhrecTags(name,{timeoutMs:LAB2_PROFILE_EDHREC_TIMEOUT_MS})
  ]);
  if(metaResult.status!=="fulfilled")throw metaResult.reason||new Error("Scryfall no pudo validar el Commander.");
  const {meta,source:metadataSource}=metaResult.value,tags=tagsResult.status==="fulfilled"?(tagsResult.value||[]):[],cards=cache.collections.get(collectionCacheKeyFor(session))||[],owned=cards.find(c=>cardNameMatches(c.name,name));
  ...
  const profile={commander:{name,typeLine:meta.typeLine||"",manaCost:meta.manaCost||"",cmc:Number(meta.cmc||0),colorIdentity:meta.colorIdentity||[],oracleText:meta.oracleText||"",image:meta.normal||meta.small||meta.large||null,imageLarge:meta.large||meta.normal||meta.small||null,ownedQuantity:Number(owned?.quantity||0)},themes,profileSources:{metadata:metadataSource,themes:tags.length?"edhrec":"local-fallback",edhrecFallback:tagsResult.status!=="fulfilled"}};
  if(session)session.lab2ProfileCache={key:profileKey,profile,updatedAt:Date.now()};
  setLab2Progress(session,"done",tags.length?"Perfil del Commander listo.":"Perfil listo · EDHREC no estuvo disponible; usando themes locales.",{commander:name,profileSources:profile.profileSources});
  return profile;
}
...
apiRouter.get("/api/cache/status",h(async(req,res)=>{
...
apiRouter.post("/api/cache/delete",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.post("/api/cache/recache",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.get("/api/cache/job",h(async(req,res)=>{
...
apiRouter.post("/api/login",h(async(req,res)=>{
...
apiRouter.post("/api/public-login",h(async(req,res)=>{
...
apiRouter.get("/api/lab3/quality/status",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.post("/api/lab3/recert/start",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});const payload=await body(req);try{const current=session.lab3RecertJob;if(current&&["planning","precheck","stress","unseen"].includes(current.status))throw new Error("Ya hay una recertificación LAB 3 en ejecución.");void runLab3Recertification(session,payload);return send(res,202,publicLab3RecertStatus(session))}catch(e){return send(res,409,{error:String(e.message||e)})}
}));
apiRouter.get("/api/lab3/recert/status",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});return send(res,200,publicLab3RecertStatus(session));
}));
apiRouter.post("/api/lab3/recert/cancel",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});const job=session.lab3RecertJob;if(!job||!["planning","precheck","stress","unseen"].includes(job.status))return send(res,409,{error:"No hay una recertificación activa."});job.cancelRequested=true;if(session.lab3FieldPrecheckJob&&session.lab3FieldPrecheckJob.status==="running")session.lab3FieldPrecheckJob.cancelRequested=true;if(session.lab3StressJob&&["planning","running"].includes(session.lab3StressJob.status))session.lab3StressJob.cancelRequested=true;if(session.lab3UnseenJob&&["planning","running"].includes(session.lab3UnseenJob.status))session.lab3UnseenJob.cancelRequested=true;job.message="Cancelación solicitada; se detendrá después del caso actual.";return send(res,200,publicLab3RecertStatus(session));
}));
apiRouter.get("/api/lab3/recert/export",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});try{const zip=await lab3RecertificationZip(session.lab3RecertJob),stamp=new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z"),name=`ManaShelf-LAB3-RECERTIFICATION-3x200x60-${stamp}.zip`;res.writeHead(200,{"Content-Type":"application/zip","Content-Disposition":`attachment; filename="${name}"`,"Content-Length":zip.length,"Cache-Control":"no-store",...SECURITY_HEADERS});res.end(zip);return}catch(e){return send(res,409,{error:String(e.message||e)})}
}));
apiRouter.post("/api/lab3/stress/start",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});
...
apiRouter.get("/api/lab3/stress/status",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});
  return send(res,200,publicStressStatus(session.lab3StressJob));
}));
apiRouter.post("/api/lab3/stress/cancel",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});
...
apiRouter.get("/api/lab3/stress/export",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 3 requiere acceso experimental."});
...
apiRouter.get("/api/lab2/access",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  return send(res,200,{unlocked:session.lab2Unlocked===true});
}));
apiRouter.post("/api/lab2/unlock",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.post("/api/lab2/profile",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  ...
  catch(e){setLab2Progress(session,"error",`Perfil detenido · ${String(e.message||e)}`,{commander:String(commander||"")});return send(res,502,{error:"LAB 2 no pudo preparar ese Commander.",detail:String(e.message||e)})}
}));
apiRouter.get("/api/lab2/progress",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 2 requiere contraseña."});
  return send(res,200,session.lab2Progress||{active:false,stage:"idle",message:"Sin procesamiento activo.",updatedAt:null});
}));
apiRouter.post("/api/lab2/build",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 2 requiere contraseña."});
  const payload=await body(req);
...
apiRouter.get("/api/lab2/build-log",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  if(session.lab2Unlocked!==true)return send(res,403,{error:"LAB 2 requiere contraseña."});
  const u=new URL(req.url,`http://${req.headers.host}`);
  const id=Number(u.searchParams.get("id"));if(!id)return send(res,400,{error:"No hay un build seleccionado."});
...
apiRouter.post("/api/lab/deck-health",h(async(req,res)=>{
...
apiRouter.post("/api/deck-detail",h(async(req,res)=>{
  const session=getSession(req);
...
apiRouter.get("/api/deck-catalog",h(async(req,res)=>{
  const session=getSession(req);
  if(!session)return send(res,401,{error:"Conectá una colección primero."});
  const u=new URL(req.url,`http://${req.headers.host}`);
...
apiRouter.post("/api/logout",h(async(req,res)=>{
  const sid=req.headers["x-manashelf-session"];
  if(sid)sessions.delete(String(sid));
  return send(res,200,{ok:true});
}));
apiRouter.get("/api/sync-status",h(async(req,res)=>{
  const session=getSession(req);
  if(!session) return send(res,401,{error:"Primero conectate con Archidekt."});
...
apiRouter.post("/api/sync-retry",h(async(req,res)=>{
  const session=getSession(req);
  if(!session)return send(res,401,{error:"Primero conectate con Archidekt."});
...
apiRouter.post("/api/decks/sync",h(async(req,res)=>{
  const session=getSession(req);
  if(!session) return send(res,401,{error:"Primero conectate con Archidekt."});
  startDeckUsageSync(session);
  const s=session.deckUsage;
...
apiRouter.get("/api/commanders",h(async(req,res)=>{
  const u=new URL(req.url,`http://${req.headers.host}`);
  return send(res,200,{results:await commanderSearch(u.searchParams.get("q")||"")});
}));
apiRouter.get("/api/collection/lookup",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.post("/api/collection",h(async(req,res)=>{
...
apiRouter.post("/api/analyze",h(async(req,res)=>{
...
apiRouter.get("/api/edhrec/tag-catalog",h(async(req,res)=>{
  return send(res,200,{tags:EDHREC_THEME_FALLBACK,source:"bundled-edhrec-tags"});
}));
apiRouter.post("/api/edhrec/tag-commanders",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.post("/api/owned-commanders/start",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  const job=startOwnedCommandersJob(session);
  return send(res,202,{job:publicOwnedCommandersJob(job)});
}));
apiRouter.get("/api/owned-commanders/status",h(async(req,res)=>{
...
apiRouter.get("/api/owned-commanders",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
  const cards=cache.collections.get(collectionCacheKeyFor(session))||[];
...
apiRouter.post("/api/compare-commanders",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.post("/api/alternatives",h(async(req,res)=>{
  const session=getSession(req);if(!session)return send(res,401,{error:"Conectá una colección primero."});
...
apiRouter.post("/api/buildability",h(async(req,res)=>{
  const session=getSession(req);
  if(!session)return send(res,401,{error:"Conectá una colección primero."});
  const {commander=""}=await body(req);
  try{
...
apiRouter.post("/api/archidekt/add-cards",h(async(req,res)=>{
...
apiRouter.post("/api/archidekt/create-deck",h(async(req,res)=>{
  const session=getSession(req);if(!session?.account)return send(res,401,{error:"Esta acción requiere iniciar sesión en Archidekt."});
...
apiRouter.use("/api",(req,res)=>send(res,404,{error:"No encontrado"}));
apiRouter.use((err,req,res,next)=>{
  if(err?.code==="PAYLOAD_TOO_LARGE")return send(res,413,{error:err.message});
  if(err?.code==="BAD_JSON")return send(res,400,{error:err.message});
  send(res,502,{error:"No pude completar la consulta.",detail:err?.message});
});
...
app.use(apiRouter);
// express.static already resolves paths safely against directory traversal (the manual
// "../" check this replaces was the v2.4.16 P0 fix; serve-static enforces the same guarantee).
app.use(express.static(PUBLIC_DIR,{index:"index.html",cacheControl:false,setHeaders:res=>res.setHeader("Cache-Control","no-cache")}));
app.use((req,res)=>{res.status(404);for(const [k,v] of Object.entries(SECURITY_HEADERS))res.setHeader(k,v);res.end("No encontrado");});

app.listen(PORT,HOST,()=>console.log(`ManaShelf v${APP_VERSION} → http://${HOST}:${PORT}`));
