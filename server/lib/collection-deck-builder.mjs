...
function comboCompleteness(mainboard,comboPackage,policy="off"){
  const normalized=normalizeComboPolicy(policy),required=normalized==="infinite";
  if(!comboPackage)return {required,selected:false,complete:!required,missing:[],reason:required?"no-complete-package":normalized==="synergistic"?"optional-no-complete-package":null};
