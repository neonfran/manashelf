...
function renderLabResult(d,target=E.labResults,inspectorMode="lab",detail=labDeckDetail){
  ...
  <section class="health-section dashboard-section" data-health-section="summary"><div class="lab-section-title"><div><span>01</span><h3>Resumen del mazo</h3></div><p>Lectura rápida antes de entrar al diagnóstico.</p></div>
    ...
    <button type="button" class="dashboard-note" data-rules-all><span class="dashboard-note-ring" style="background:${rulesRingGradient}"><em>${rulesOk}/${rulesTotal}</em></span><span class="dashboard-note-copy"><strong>Fundamentos</strong><span>${rulesOk}/${rulesTotal} sin alertas</span><small>${rulesTotal} reglas evaluadas</small></span></button>
    <div class="visual-dashboard">
      ...
      <div class="theme-chart"><div class="mana-curve-head"><strong>Temáticas ${labInfo("Temas detectados por evidencia EDHREC/Oracle. % = cartas que sostienen ese tema sobre el tamaño total del mazo (Size).")}</strong><small>Temas inferidos</small></div><div class="theme-chart-body">${d.themes.length?d.themes.map((t,ti)=>{const filled=Math.max(1,Math.round(Math.min(100,t.density)/10));const color=THEME_BAR_COLORS[ti%THEME_BAR_COLORS.length];return `<button type="button" class="theme-bar-row" data-theme-ref="${ti}"><span class="theme-bar-name">${esc(t.name)}</span><span class="theme-segments">${Array.from({length:10},(_,i)=>`<i class="${i<filled?"on":""}" style="${i<filled?`background:${color};color:${color}`:""}"></i>`).join("")}</span><b class="theme-bar-pct">${t.density}%</b></button>`}).join(""):'<p class="lab-muted">Sin temáticas con evidencia suficiente.</p>'}</div></div>
  ...
  <section class="health-section structural-health-section" data-health-section="health"><div class="lab-section-title"><div><span>02</span><h3>Salud estructural</h3></div><p>Los valores auditan las cartas detectadas en la lista lateral.</p></div><div class="health-cards">${health}</div></section>
  <section class="health-section rules-section" data-health-section="health"><div class="lab-section-title"><div><span>03</span><h3>Recomendaciones estructurales</h3></div><p>Se evalúan todas las reglas y, si aplican varias, se muestran todas.</p></div>${gaps.trim()?`<div class="rules-callouts"><h4>A considerar</h4><div class="gap-grid">${gaps}</div></div>`:""}<div class="rules-strip-head"><h4>${(d.structuralRules||[]).length} reglas evaluadas</h4><small>Referencia completa, no solo lo que necesita atención</small></div><div class="structural-rule-list">${structuralRules}</div></section>
  <section class="health-section identity-section" data-health-section="health"><div class="lab-section-title"><div><span>04</span><h3>Identidad del mazo</h3></div><p>Cada resultado muestra cuántas cartas concretas lo sostienen.</p></div><div class="theme-grid-v2">${themes}</div>${themeCoreHtml}</section>
