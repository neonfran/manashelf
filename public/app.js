...
async function lab3ProfileProgressOnce(){
  const p=await req("/api/lab3/progress",{headers:authHeaders()}).catch(()=>null);if(!p?.message||!p.active)return;
  const small=E.lab3CommanderCard?.querySelector("small"),loading=E.lab3ThemeGrid?.querySelector(".lab2-inline-loading span");if(small)small.textContent=p.message;if(loading)loading.textContent=p.message;
}
async function chooseLab3Commander(c){
  clearError();lab3Commander=c;lab3Profile=null;lab3Theme=null;resetLab3Result();E.lab3CommanderSearch.value=c.name;E.lab3CommanderDropdown.classList.add("hidden");E.lab3ThemeStep.classList.remove("hidden");E.lab3Settings.classList.add("hidden");E.lab3ThemeGrid.innerHTML='<div class="lab2-inline-loading"><i></i><span>Validando Commander en Semantic DB…</span></div>';terminalSetActivity(`perfil LAB 3 · ${c.name}`);
  ...
  let tick=null;
  try{
    tick=setInterval(lab3ProfileProgressOnce,350);void lab3ProfileProgressOnce();
    lab3Profile=await req("/api/lab3/profile",{method:"POST",headers:authHeaders(),body:JSON.stringify({commander:c.name})});
    const pc=lab3Profile.commander||c;lab3Commander={...c,...pc,image:pc.image||c.image,largeImage:pc.imageLarge||c.largeImage};
    E.lab3CommanderCard.innerHTML=`${lab3Commander.image?`<img src="${esc(lab3Commander.image)}" alt="${esc(lab3Commander.name)}">`:""}<div><h3>${esc(lab3Commander.name)}</h3><p>${esc(lab3Commander.manaCost||"")} · ${esc(lab3Commander.typeLine||"")}</p><small>${Number(lab3Commander.ownedQuantity||0)>0?`EN TU COLECCIÓN · ${Number(lab3Commander.ownedQuantity)} copia${Number(lab3Commander.ownedQuantity)===1?"":"s"}`:"COMMANDER NO POSEÍDO · el 99 se arma desde tu colección"} · ${esc(lab3Commander.semanticStatus||"semantic")}</small></div>`;
    E.lab3CommanderCard.onclick=()=>commanderModal(lab3Commander);wireCommanderPreview(E.lab3CommanderCard.querySelector("img"),lab3Commander,lab3Commander.image||"");renderLab3Themes();E.lab3Settings.classList.remove("hidden");E.lab3Generate.disabled=true;E.lab3BuildHint.textContent="Elegí un theme para continuar.";terminalClearActivity();terminalShowSelection("LAB 3 Commander",lab3Commander.name);
  }catch(e){terminalClearActivity();E.lab3ThemeGrid.innerHTML="";E.lab3ThemeStep.classList.add("hidden");showError(e)}
...
async function startLab3Stress(){clearError();try{const s=await req("/api/lab3/stress/start",{method:"POST",headers:authHeaders(),body:JSON.stringify({runs:200,seed:"manashelf-lab3-stress-4"})});renderLab3Stress(s);if(lab3StressTimer)clearInterval(lab3StressTimer);lab3StressTimer=setInterval(pollLab3Stress,1000);void pollLab3Stress()}catch(e){showError(e)}}
