...
export default function Drawer() {
  ...
  const applyToDeck = async () => {
    ...
    if (!confirm(t(`¿Agregar ${names.length} cartas a "${activeDeckDetail.name}" en Archidekt? Esta acción modifica el deck real.`))) return;
    try {
      const d = await archidektAddCards({ deckId: activeDeckDetail.id, names, confirm: "CONFIRMAR" });
      alert(t(`Archidekt actualizado: ${d.affected} cartas agregadas.`));
    } catch (e) {
      showError(e);
    }
  };
  ...
  return (
    <div className="drawer">
      <div className="drawer-head">
        <strong>{drawerOpen === "shortlist" ? t(`Shortlist · ${shortlist.length}`) : t("Historial")}</strong>
        ...
        {drawerOpen === "shortlist" ? (
          <>
            ...
            {shortlist.map((x) => (
              <div className="drawer-item" key={x.name}>
                <div><strong>{x.name}</strong><small>{x.role} · {x.owned ? t(`${x.available} disponible${x.available === 1 ? "" : "s"}`) : t("No está en tu colección")}</small></div>
          ...
          history.map((h, i) => (
            <div className="drawer-item" key={i}>
              ...
              <span>{t(`${h.summary?.recommendedOwned || 0} propias`)}</span>
            </div>
          ))
        ) : (
          <p className="status">{t("Todavía no hay análisis guardados.")}</p>
        )}
      </div>
    </div>
  );
