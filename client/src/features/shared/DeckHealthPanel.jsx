  ...
  return (
    <article className="health-card">
      ...
      <div className={`health-meter ${key(h.level)}`}>
        {markerPct != null && <span className="health-meter-marker" style={{ left: `${markerPct}%` }} title="Piso orientativo"></span>}
...
export default function DeckHealthPanel({ data: d, detail, onAudit, showMetrics = false, sections = ALL_SECTIONS }) {
  ...
  return (
    <div>
      <div className="lab-result-head"><div><span>{t("SALUD DEL MAZO · EXPERIMENTAL")} <InfoDot text={`Método: cuatro ejes estructurales, clasificación heurística por texto Oracle y CMC medio ${d.context.avgCmc}. No conoce tu metajuego ni intención exacta.`} /></span><h2>{d.deck.name}</h2><p>{d.deck.commander || "Commander no detectado"} · Size {d.deck.size}</p></div></div>

      {show("summary") && <section className="health-section dashboard-section" data-lab-section="summary">
        <div className="lab-section-title"><div><span>01</span><h3>{t("Resumen del mazo")}</h3></div><p>Lectura rápida antes de entrar al diagnóstico.</p></div>
        <div className="deck-dashboard">
          <button className="dashboard-stat" onClick={() => onAudit(d.health?.[0]?.refs?.[0]?.cards || [], "Tierras")}><b>{d.context.lands ?? "—"}</b><span>Tierras</span></button>
          <button className="dashboard-stat" onClick={() => onAudit(d.health?.[0]?.refs?.[1]?.cards || [], "Ramp")}><b>{d.context.ramp ?? "—"}</b><span>Ramp</span></button>
          <button className="dashboard-stat" onClick={() => onAudit(d.health?.[1]?.refs?.[0]?.cards || [], "Card Advantage")}><b>{d.context.draw ?? "—"}</b><span>Card Advantage</span></button>
          <button className="dashboard-stat" onClick={() => onAudit((d.health?.[2]?.refs || []).flatMap((r) => r.cards || []), "Interacción")}><b>{d.context.interaction ?? "—"}</b><span>Interacción</span></button>
          <button className="dashboard-stat" onClick={() => onAudit(d.health?.[2]?.refs?.[2]?.cards || [], "Board wipes")}><b>{d.context.wipes ?? "—"}</b><span>Board wipes</span></button>
          <div className="dashboard-stat"><b>{Number(d.context.avgCmc || 0).toFixed(1)}</b><span>CMC medio</span></div>
        </div>
        <button type="button" className="dashboard-note" onClick={() => onAudit((d.structuralRules || []).flatMap((r) => r.cards || []), "Criterios evaluados")}>
          <span className="dashboard-note-ring" style={{ background: rulesRingGradient }}><em>{rulesOk}/{rulesTotal}</em></span>
          <span className="dashboard-note-copy"><strong>Fundamentos</strong><span>{rulesOk}/{rulesTotal} sin alertas</span><small>{rulesTotal} criterios evaluados</small></span>
        </button>
        <div className="visual-dashboard">
          <div className="mana-curve compact">
            <div className="mana-curve-head"><strong>{t("Curva de maná")} <InfoDot text="Cartas no-tierra agrupadas por coste de maná convertido (CMC), coloreadas por tipo primario." /></strong><small>Cartas según Mana Value (CMC)</small></div>
            <div className="curve-type-legend">{MTG_CURVE_TYPES.map((tp) => (
              <button type="button" key={tp} onClick={() => onAudit((d.context.curve || []).flatMap((x) => x.cardsByType?.[tp] || []), `Curva · ${tp}`)}><i style={{ background: MTG_TYPE_COLORS[tp] }}></i>{tp}</button>
            ))}</div>
            <div className="mana-curve-plot">
              <div className="curve-axis-labels"><span>Cartas</span><span>CMC</span></div>
                ...
                {(d.context.curve || []).map((x, ci) => {
                  ...
                  return (
                    <div className="curve-col" key={ci}>
                      ...
                      <button type="button" className="curve-all" title={`Ver todas las cartas de CMC ${x.cmc}`} onClick={() => onAudit(x.cards || [], `CMC ${x.cmc}`)}>{x.cmc}</button>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="theme-chart">
            <div className="mana-curve-head"><strong>{t("Temáticas")} <InfoDot text="Temas detectados por evidencia EDHREC/Oracle. % = cartas que sostienen ese tema sobre el tamaño total del mazo (Size)." /></strong><small>Temas inferidos</small></div>
            <div className="theme-chart-body">
              {d.themes.length ? d.themes.map((th, ti) => {
                ...
                return (
                  <button type="button" className="theme-bar-row" key={ti} onClick={() => onAudit(th.cards || [], `${th.name} · ${th.cardCount || th.cards?.length || 0} cartas detectadas`)}>
              ...
              }) : <p className="lab-muted">Sin temáticas con evidencia suficiente.</p>}
            </div>
          </div>
          <div className="type-chart">
            <div className="mana-curve-head"><strong>{t("Tipos de carta")} <InfoDot text="Distribución del mazo por tipo primario de carta (Creature, Instant, etc.), sin contar tierras." /></strong><small>Tipo primario</small></div>
            <div className="type-chart-body">
              <button type="button" className="type-donut" style={{ background: donutGradient(d.context.typeDistribution || []) }} onClick={() => onAudit(null, "")}>
                <span>{(d.context.typeDistribution || []).reduce((n, x) => n + x.count, 0)}</span><small>cartas</small>
              </button>
              <div className="type-legend">
                {(d.context.typeDistribution || []).map((x) => (
                  <button key={x.type} onClick={() => onAudit(cardsByTypeLine(detail, x.type), `Tipo · ${x.type}`)}><i className="type-dot" style={{ background: MTG_TYPE_COLORS[x.type] || MTG_TYPE_COLORS.Other }}></i><span>{x.type}</span><b>{x.count}</b></button>
                ))}
              </div>
            </div>
          </div>
        </div>
        {d.edhrecWarning && <div className="health-degraded"><strong>{t("Análisis local completo.")}</strong><span>{d.edhrecWarning} Themes e inclusiones pueden quedar vacíos hasta que EDHREC responda.</span></div>}
      </section>}

      {show("health") && <section className="health-section structural-health-section" data-lab-section="health">
        <div className="lab-section-title"><div><span>02</span><h3>{t("Salud estructural")}</h3></div><p>Los valores auditan las cartas detectadas en la lista lateral.</p></div>
      ...
      {show("rules") && <section className="health-section rules-section" data-lab-section="rules">
        <div className="lab-section-title"><div><span>03</span><h3>{t("Estructura del mazo")}</h3></div><p>Se evalúan los criterios estructurales del mazo y se señalan los puntos que merecen atención.</p></div>
        {(d.gaps || []).length > 0 && (
          <div className="rules-callouts"><h4>A considerar</h4><div className="gap-grid">
            {d.gaps.map((g, i) => (
        ...
        <div className="rules-strip-head"><h4>{(d.structuralRules || []).length} {t("criterios evaluados")}</h4><small>Referencia completa, no solo lo que necesita atención</small></div>
        <div className="structural-rule-list">
          {(d.structuralRules || []).map((r, ri) => (
            <button type="button" key={ri} className={`structural-rule ${r.triggered ? "triggered" : "ok"}`} disabled={!r.cards?.length} onClick={() => onAudit(r.cards, `Criterio · ${r.type}`)}>
      ...
      {show("identity") && <section className="health-section identity-section" data-lab-section="identity">
        <div className="lab-section-title"><div><span>04</span><h3>{t("Identidad del mazo")}</h3></div><p>Cada resultado muestra cuántas cartas concretas lo sostienen.</p></div>
        <div className="theme-grid-v2">
          {d.themes.length ? d.themes.map((th, ti) => {
            const sample = (th.cards || []).slice(0, 4);
            const rest = (th.cards || []).length - sample.length;
            return (
              <article className="theme-card theme-click" key={ti} tabIndex={0} onClick={() => onAudit(th.cards || [], `${th.name} · ${th.cardCount || th.cards?.length || 0} cartas detectadas`)}>
                <div><b>{th.name}</b><span className="theme-tier">{th.tier || "Theme"}</span><span>Confianza {th.confidence}</span><InfoDot text={th.explanation} /></div>
                ...
                <p><strong>{th.cardCount || th.cards?.length || 0} cartas detectadas</strong> que apoyan {th.name} · ~{th.density}% del deck{th.commanderEvidence ? " · Commander compatible" : ""}</p>
                {sample.length > 0 && <div className="theme-card-sample">{sample.map((n) => <span key={n}>{n}</span>)}{rest > 0 && <span className="more">+{rest} más</span>}</div>}
              </article>
            );
          }) : <p className="lab-muted">No hay evidencia suficiente para inferir un tema dominante. ManaShelf prefiere no inventarlo.</p>}
        </div>
        {themeCore.length > 0 && (
          <div className="theme-core">
            <div><b>Cartas que atan varias temáticas</b><span>Sostienen 2 o más de las temáticas de arriba a la vez — suelen ser el corazón funcional del mazo.</span></div>
            ...
            <article className="swap-pair" key={si}>
              <div className="swap-pair-row">
                <div className="swap-mini in">
                  ...
                  <small>IN</small><strong>{x.include.name}</strong><p>{x.include.reason || x.include.inclusionType || "Recomendación contextual"}</p>
                </div>
                <div className="swap-connector" data-confidence={key(x.confidence)}><span className="swap-arrow">→</span><b>{x.confidence}</b><small>confianza</small></div>
                <div className="swap-mini out">
                  ...
                  <small>OUT</small>
                  {x.cut ? <><strong>{x.cut.name}</strong><p>{(x.cut.reasons || []).slice(0, 2).join(" · ")}</p></> : <><strong>Sin corte claro</strong><p>Prefiero no proponer un cambio sin evidencia suficiente.</p></>}
                </div>
              </div>
              <footer><span>{(x.pairReasons || []).join(" · ") || "Mejor combinación contextual disponible."}</span>{x.impact && <div className="swap-impact"><span>CMC {x.impact.avgCmcDelta > 0 ? "+" : ""}{x.impact.avgCmcDelta}</span><span>Roles críticos protegidos ✓</span></div>}</footer>
            </article>
          )) : <div className="empty-cut"><strong>{t("No encuentro un corte claro.")}</strong><span>No hay suficiente evidencia para recomendar una salida sin arriesgar la estructura del mazo.</span></div>}
        </div>
