...
export function Lab2Summary({ d }) {
  ...
  const copyDetail = blocked ? `${blocked} candidatas bloqueadas por otros mazos` : `${Number(s.occupiedCards || 0)} slots ocupados en la lista final`;
  return (
    <div className="lab2-summary-grid">
      <div className="lab2-summary-card"><span>SIZE</span><strong>{b.size}/100</strong><small>{b.complete ? "Lista completa" : "Colección insuficiente"}</small></div>
      <div className="lab2-summary-card"><span>THEME</span><strong>{pct(s.themeDensity || 0)}</strong><small>{Number(s.themeCards || 0)} cartas · objetivo {Number(targets.theme || 0)}</small></div>
      <div className="lab2-summary-card"><span>TIERRAS</span><strong>{Number(s.lands || 0)}</strong><small>objetivo {Number(targets.lands || 0)} · MV {Number(s.avgCmc || 0).toFixed(2)}</small></div>
      <div className="lab2-summary-card"><span>ESTRUCTURA</span><strong>{fmtNum(Number(roles.ramp || 0), 1)} / {fmtNum(Number(roles.interaction || 0), 1)}</strong><small>ramp fiable / interacción</small></div>
      <div className="lab2-summary-card"><span>COPIAS</span><strong>{Number(s.freeCards || 0)}</strong><small>{copyDetail}</small></div>
      {combo && <div className="lab2-summary-card"><span>COMBO</span><strong>{combo.infinite ? "∞" : "✓"}</strong><small>{Number(combo.pieces?.length || 0)} piezas · {combo.infinite ? "infinito" : "completo"}</small></div>}
    </div>
  );
}
...
export function Lab2Warnings({ d }) {
  ...
  return (
    <div className="lab2-warnings">
      <div className="lab2-warning-head"><strong>{rows.some((x) => x.severity === "high") ? "Compromisos detectados" : "Notas del armado"}</strong><span>{rows.length}</span></div>
      {rows.map((x, i) => <div className={`lab2-warning-row ${x.severity || "info"}`} key={i}><b>{x.label || "Nota"}</b><p>{x.message || x}</p></div>)}
    </div>
  );
}
...
export function Lab2Audit({ d }) {
  ...
  const sourceRows = colors.filter((c) => Number(mana.demand?.share?.[c] || 0) > 0 || Number(mana.effectiveSources?.[c] ?? mana.sources?.[c] ?? 0) > 0).map((c) => {
    ...
    const hint = hard ? `${hard.card} · ${hard.pips} pip${Number(hard.pips) === 1 ? "" : "s"} · T${hard.turn}` : "sin requisito temprano crítico";
    return { c, sources, landSources, support, wanted, prob, hint };
  });
  const landReason = adj.reasons?.length ? adj.reasons.join(" · ") : "sin ajuste adicional después del primer armado";
    ...
    <div>
      <div className="lab2-audit-grid">
        <section>
          ...
          <h4 className="lab2-subhead">Balance por tipo</h4>
          <div className="lab2-target-list">{typeRows.map((x) => <div key={x.name}><span>{x.name}</span><b className={x.under ? "under" : "ok"}>{x.actual}{x.desired ? ` · objetivo ~${x.desired}` : ""}{x.floor ? ` · piso ${x.floor}` : ""}{x.cap ? ` · tope ${x.cap}` : ""}</b></div>)}</div>
        </section>
        <section>
          <h4>Base de maná</h4>
          <p>Objetivo inicial {Number(adj.baseTarget ?? t.lands ?? 0)} → final <b>{Number(t.lands || 0)}</b>. {landReason}.</p>
          <div className="lab2-mana-meta">
            <span><b>{basic}</b><small>básicas</small></span><span><b>{nonbasic}</b><small>no básicas</small></span><span><b>{fetches}</b><small>fetches</small></span><span><b>{manaRepairs}</b><small>swaps de fixing</small></span><span><b>{Math.round(Number(landDrops.turn3 || 0) * 100)}%</b><small>3 tierras en T3</small></span>
          </div>
          <div className="lab2-mana-sources">
            {sourceRows.length ? sourceRows.map((x) => (
              <span title={x.hint} key={x.c}><b>{x.c}</b><strong>{fmtNum(x.sources, 1)}{x.wanted ? ` / ${x.wanted}` : ""}</strong><small>{fmtNum(x.landSources, 1)} tierras{x.support ? ` + ${fmtNum(x.support, 1)} soporte temprano` : ""} · {x.prob}% cast</small></span>
            )) : <span><small>Sin demanda de color detectada.</small></span>}
          </div>
        </section>
      </div>
      {combo && (
        <section className="lab2-combo-audit">
          ...
          <p><b>{combo.infinite ? "Combo infinito" : "Combo completo"}</b> · {combo.pieces.map((p) => p.name).join(" + ")}</p>
          {combo.produces?.length > 0 && <p>Resultado: {combo.produces.map((x) => x.name || x).join(" · ")}</p>}
          {combo.manaNeeded && <p>Maná requerido: {String(combo.manaNeeded)}</p>}
        </section>
      )}
...
export function lab2AvailabilityLabel(c, lab2DeckDetail, lab2Profile) {
...
export function Lab2DeckTable({ detail, profile }) {
  ...
  return (
    <table className="lab2-deck-table">
      <thead><tr><th>CARTA</th><th>TIPO</th><th>CATEGORÍA</th><th>CMC</th><th>DISPONIBILIDAD</th></tr></thead>
      <tbody>
        {cards.map((c) => (
          <tr key={c.name}>
            ...
            <td>{lab2AvailabilityLabel(c, detail, profile)}</td>
          </tr>
        ))}
      </tbody>
    </table>
