...
import { BRACKET_OPTIONS, BRACKET_INFO_TEXT, BRACKET_SETTING_DESCRIPTION } from "../shared/brackets.js";

const DEFAULT_SETTINGS = { themeFocus: 72, ramp: "standard", interaction: "standard", curve: "normal", synergyBias: "balanced", commanderDependence: "normal", comboPolicy: "off", landStyle: "balanced", protectExistingDecks: true, bracket: "none", forceGameChangers: false };
...
export default function Lab2Flow() {
  ...
  useEffect(() => {
    if (profileLoading) setActivity(`DeckBuilder Classic · ${profileMessage}`);
    else if (building) setActivity(`DeckBuilder Classic · ${buildMessage}`);
    else clearActivity();
  ...
  const exportArchidekt = () => { if (!result?.deckDetail) return showError(new Error(t("Generá un mazo primero."))); download(`${safeName(result.deckDetail.name)}-archidekt.txt`, archidektText()); };
  const copyArchidekt = async () => {
    if (!result?.deckDetail) return showError(new Error(t("Generá un mazo primero.")));
    try {
  ...
  const exportLog = async () => {
    if (!result?.diagnosticLogId) return showError(new Error(t("No hay un log de armado disponible.")));
    try {
  ...
  return (
    <section className="flow-panel lab2-panel">
      <div className="lab2-banner"><b>⚗ DECKBUILDER CLASSIC</b><span>{t("MOTOR CLÁSICO · CONSTRUYE DESDE TU COLECCIÓN · NO MODIFICA ARCHIDEKT")}</span></div>
      <div className="flow-head"><span>{t("BUILD FROM COLLECTION")}</span><p>{t("Elegí un Commander, seleccioná uno de sus themes y ManaShelf arma un deck coherente usando las mejores cartas legales disponibles en tu colección.")}</p></div>
          ...
          <div className="chosen lab2-commander-chosen">
            <div className="chosen-card commander-click" onClick={() => openModal("commander", commander)}>
              ...
              <div>
                ...
                <small>{profileLoading ? t(profileMessage) : Number(commander.ownedQuantity || 0) > 0 ? t(`EN TU COLECCIÓN · ${commander.ownedQuantity} copia${commander.ownedQuantity === 1 ? "" : "s"}`) : t("COMMANDER NO POSEÍDO · el 99 se arma desde tu colección")}</small>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="lab2-theme-grid">
            {(profile.themes || []).length ? profile.themes.map((th, i) => (
              <button type="button" key={i} className={`lab2-theme-choice${theme?.slug === th.slug ? " active" : ""}`} title={minimalThemeDescription(th)} onClick={() => chooseTheme(th)}>
                <strong>{th.name}</strong><small>{th.fallback ? t("Fallback balanceado") : th.localFallback ? t("Inferido del Commander") : t(`${Number(th.count || 0).toLocaleString()} decks EDHREC`)}</small>
              </button>
            )) : <p className="lab-muted">{t("No encontré themes disponibles para este Commander.")}</p>}
          </div>
        </div>
      )}
      ...
      {profile && (
        <div className="lab2-step">
          ...
          <div className="lab2-settings-grid">
            ...
            <label className="lab2-setting"><span><strong>{t("Interacción")}</strong><small>{t("Cuánto espacio dedicar a removal, counters, wipes y otras respuestas.")}</small></span><select value={settings.interaction} onChange={(e) => setSettings((s) => ({ ...s, interaction: e.target.value }))}><option value="less">{t("Menos respuestas")}</option><option value="standard">{t("Estándar")}</option><option value="more">{t("Más respuestas")}</option></select></label>
            ...
            <label className="lab2-setting"><span><strong>{t("Dependencia del Commander")}</strong><small>{t("Cuánto aceptar cartas cuyo rendimiento baja si el Commander no está disponible.")}</small></span><select value={settings.commanderDependence} onChange={(e) => setSettings((s) => ({ ...s, commanderDependence: e.target.value }))}><option value="conservative">{t("Baja dependencia")}</option><option value="normal">{t("Balanceada")}</option><option value="all-in">{t("Alta dependencia")}</option></select></label>
            ...
            <label className="lab2-setting"><span><strong>{t("Bracket (nivel de poder)")}</strong> <InfoDot text={BRACKET_INFO_TEXT} /><small>{t(BRACKET_SETTING_DESCRIPTION)}</small></span><select value={settings.bracket} onChange={(e) => setSettings((s) => ({ ...s, bracket: e.target.value }))}>{BRACKET_OPTIONS.map((b) => <option key={b.value} value={b.value}>{t(b.label)}</option>)}</select></label>
            <label className="lab2-setting lab2-switch-setting"><span><strong>{t("Forzar Game Changers permitidos")}</strong><small>{t("Mete los Game Changers de mayor poder que tengas hasta el tope de tu bracket, aunque no encajen con el theme. Hoy solo cambia algo en Bracket 3 · Mejorado (tope de 3); en el resto no hay tope numérico para forzar.")}</small></span><input type="checkbox" checked={settings.forceGameChangers} onChange={(e) => setSettings((s) => ({ ...s, forceGameChangers: e.target.checked }))} /><i aria-hidden="true"></i></label>
      ...
      {result && !building && (
        <div>
          ...
          <div className="lab2-health-workspace">
            ...
            <DeckInspector id="lab2DeckInspector" detail={result.deckDetail} filter={inspectorFilter} onClearFilter={() => setInspectorFilter(null)} title={t("Mazo generado")} showCategory />
          </div>
        </div>
      )}
    </section>
