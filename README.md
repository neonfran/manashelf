...
> Beta vigente para testing: **v2.5.57-beta**.
...
## Documentación viva
La documentación vigente vive en [`docs/README.md`](docs/README.md). Incluye especificación funcional, arquitectura con diagramas Mermaid, contrato detallado de LAB 2, decisiones técnicas y estrategia de testing/diagnóstico. `npm run test:docs` evita publicar una release con la baseline documental principal desactualizada.
...
- Tanto el acceso público como el privado sincronizan en segundo plano el contenido accesible de los mazos para calcular tamaño y uso cruzado de cartas; el privado además puede incluir mazos no públicos y habilita escrituras confirmadas.
...
- Si LAB 2 usa una política de combo, el servidor consulta Commander Spellbook para obtener variantes del Commander; si ese servicio no responde, el build continúa sin combo y muestra una advertencia.
...
- **LAB 2 · Build From Collection (experimental):** genera un deck Commander orientado a un theme elegido usando el pool legal de la colección, perfiles de arquetipo, pisos/topes estructurales por tipo, básicas ilimitadas, base de maná con demanda de colores/fuentes y una auditoría Deck Health/Deck Metrics posterior. Puede buscar paquetes de combo exactos en Commander Spellbook (`Sin combos / Sinérgicos / Infinito`), priorizar una base `Básicas primero`, exportar TXT listo para Archidekt y un log JSON diagnóstico; no escribe directamente en Archidekt.
- **LAB 3 · Semantic Builder / Builder vNext (candidate):** Builder v9 consume Semantic v2 + Deck Context v6 + Mana Model v4 mediante la boundary vNext. Excluye impresiones no jugables, exige legalidad Commander, usa Theme Understanding v4 con fallback conservador y queda protegido por Knowledge integrity, Stress/unseen gates y Promotion Gate. LAB2 sigue disponible como baseline hasta reunir suficiente evidencia A/B actual.
...
LAB3 requiere que el deploy incluya el directorio `data/` con `lab3-runtime-index.jsonl.gz`, `lab3-theme-corpus.json.gz` y `lab3-knowledge-db.json`. No son cachés opcionales: son el snapshot de conocimiento certificado. En producción ManaShelf valida esos tres assets y sus SHA antes de abrir el puerto; un deploy incompleto falla de forma explícita en vez de dejar LAB3 roto en runtime. Ejecutar `npm run test:deployment` antes de publicar.
...
LAB3 includes a diagnostic-only **200-build stress corpus** for engine hardening. Open LAB3 and use **Ejecutar 200 builds**; after completion export the single `ManaShelf-LAB3-STRESS-CORPUS-*.zip` file and use it for replay/regression analysis. See `docs/LAB3_STRESS_HARNESS.md`.
