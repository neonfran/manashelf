...
export const CONTEXT_OPTIMIZER_VERSION=1;
...
export function deriveContextProfile(card){
  ...
  const roles=card?.views?.roles||deriveRoleViews(card),dependency=card?.views?.dependency||deriveDependencyView(card),theme=card?.views?.theme||deriveThemeView(card),mana=card?.views?.mana||deriveManaView(card),castability=card?.views?.castability||deriveCastabilityView(card);
...
function commanderLink(profile,commanderProfile){
  if(!commanderProfile)return {score:0,helpsCommander:[],helpedByCommander:[]};
  const helps=[],helped=[];
  for(const need of commanderProfile.dependency.externalNeeds)if(profileSupports(profile,need)>0)helps.push(need);
  for(const need of profile.dependency.externalNeeds)if(profileSupports(commanderProfile,need)>0)helped.push(need);
  const score=clamp((helps.length+helped.length)/Math.max(1,commanderProfile.dependency.externalNeeds.length+profile.dependency.externalNeeds.length));
  return {score,helpsCommander:uniq(helps),helpedByCommander:uniq(helped)};
}
...
export function buildContextState(cards,{commander=null,themeFacet=null,roleTargets={},policy=DEFAULT_CONTEXT_POLICY}={}){
  ...
  const cardsOut=profiles.map(p=>{
    ...
    const commanderSynergy=commanderLink(p,commanderProfile),tfit=themeFit(p,themeFacet),rneed=roleNeed(p,roleCoverage,roleTargets),pressure=castPressure(p,colorSources);
