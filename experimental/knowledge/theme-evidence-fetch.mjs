...
async function getJson(url,{fetchImpl,retries=2,retryDelayMs=500}={}){
  let last=null;
  for(let attempt=0;attempt<=retries;attempt++){
    try{const r=await fetchImpl(url);if(r?.ok)return await r.json();last=new Error(`HTTP ${r?.status??"unknown"} for ${url}`);if(Number(r?.status||0)===404)throw Object.assign(last,{nonRetryable:true});}
...
export async function fetchEdhrecTrainingSnapshot({plan,fetchImpl=globalThis.fetch,themesPerCommander=6,minThemeDecks=5,requestDelayMs=250,retries=2,resume=null,onCheckpoint=null}={}){
  ...
  for(const commander of plan.commanders){
    const cid=String(commander.oracleId);if(completed.has(cid))continue;
    const baseUrl=`https://json.edhrec.com/pages/commanders/${edhrecSlug(commander.name)}.json`;let basePayload;
    try{basePayload=await getJson(baseUrl,{fetchImpl,retries});failureMap.delete(`base|${cid}|`);}
    catch(e){const f={commander:commander.name,commanderOracleId:cid,stage:"base",retryable:!e?.nonRetryable,error:String(e.message||e)};failureMap.set(failureKey(f),f);if(e?.nonRetryable)completed.add(cid);if(onCheckpoint)await onCheckpoint(snapshot());continue;}
    ...
    for(const tag of tags){
      ...
      try{await sleep(requestDelayMs);const themePayload=await getJson(themeUrl,{fetchImpl,retries});sampleMap.set(sourceId,{sourceId,commander:commander.name,commanderOracleId:cid,colorIdentity:commander.colorIdentity||[],theme:tag.name,themeSlug:slug,themeCount:Number(tag.count||0),baseLists,themeLists:parseEdhrecLists(themePayload)});failureMap.delete(`theme|${cid}|${slug}`);}
      catch(e){const f={commander:commander.name,commanderOracleId:cid,theme:tag.name,themeSlug:slug,stage:"theme",retryable:!e?.nonRetryable,error:String(e.message||e)};failureMap.set(failureKey(f),f);if(!e?.nonRetryable)retryableThemeFailure=true;}
    }
