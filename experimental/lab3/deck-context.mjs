import { buildContextState as buildSemanticContextState, deriveContextProfile, evaluateProfileDependencies, evaluateProfileSupport } from "../semantic-v2/context-optimizer.mjs";
import { COVERAGE } from "../semantic-v2/schema.mjs";
import { scoreCardForThemeModel, dominantThemeFeature } from "./theme-understanding.mjs";

export const LAB3_CONTEXT_ENGINE_VERSION=4;
...
function exactCommanderSynergy(profile,commanderProfile){
  if(!commanderProfile)return 0;const cp=new Set(commanderProfile.dependency.produces||[]),pp=new Set(profile.dependency.produces||[]),helps=(commanderProfile.dependency.externalNeeds||[]).filter(x=>pp.has(x)).length,helped=(profile.dependency.externalNeeds||[]).filter(x=>cp.has(x)).length;return clamp01((helps+helped)*.24);
}
...
export function cardProfile(card,{theme="",themeMode="semantic",themeModel=null,commander=null,external={}}={}){
  ...
  const directSemantic=themeContract.adjudication==="positive"?Number(themeContract.score||0):0,inferredSemantic=themeModel?.mode==="semantic_inferred"?scoreCardForThemeModel(card,themeModel):0,semanticTheme=clamp01(Math.max(directSemantic,inferredSemantic)),externalTheme=clamp01(Number(external.themeAffinity||external.edhrecThemeScore||0)),externalCommander=clamp01(Number(external.commanderAffinity||external.edhrecBaseScore||0));
  ...
  const themeScore=externalFallback?clamp01(fallbackThemeScore+semanticTheme*.12):effectiveMode==="semantic_inferred"?clamp01(semanticTheme*.72+externalTheme*.28):clamp01(semanticTheme*.82+externalTheme*.18),themeEvidenceSource=externalFallback?(externalTheme>0?"edhrec_external_fallback":"none"):(effectiveMode==="semantic_inferred"?(semanticTheme>0&&externalTheme>0?"semantic_inferred+edhrec":semanticTheme>0?"semantic_inferred":externalTheme>0?"edhrec_rank":"none"):(semanticTheme>0&&externalTheme>0?"semantic+edhrec":semanticTheme>0?"semantic":externalTheme>0?"edhrec_rank":"none"));
  const inferredFacet=effectiveMode==="semantic_inferred"?dominantThemeFeature(card,themeModel):null,commanderProfile=commander?deriveContextProfile(commander):null,directCommanderSynergy=clamp01(exactCommanderSynergy(semantic,commanderProfile)+externalCommander*.3);
  return {card,semantic,themeContract,themeMode:effectiveMode,themeFacet:support.facet||(inferredFacet?`inferred:${inferredFacet}`:null),themeEvidenceSource,roleScores:semantic.roleScores,semanticTheme,externalTheme,themeScore,statusWeight:semantic.coverageWeight,produces:uniq(semantic.dependency.produces),needs:uniq(semantic.dependency.externalNeeds),types:[...types],subtypes:[...subtypes],directCommanderSynergy,external};
}
...
export function analyzeDeckContext(profiles,{commander=null,theme="",roleTargets={}}={}){
  ...
  return {...state,cards,packageLinks:links,summary:{...state.summary,avgDependencySatisfaction:state.summary.avgDependencyReliability,avgDeadCardRisk:cards.length?cards.reduce((n,x)=>n+x.deadCardRisk,0)/cards.length:0,avgContextualSynergy:state.summary.avgStructuralScore,dependencyCoverage:state.summary.avgDependencyReliability,dependencyRedundancy:redundancy,packageLinks:links.length,commanderDependence:cards.length?cards.filter(x=>x.dependency?.summary?.commanderDependent).length/cards.length:0}};
}
